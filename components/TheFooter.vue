...
const navigation = ref<{ url: string, icon: string }[]>([
  {
...
<template>
  <footer class="fixed bottom-0 left-0 lg:static w-full bg-background lg:bg-transparent z-10 lg:mb-10">
    <TheContainer class="p-4">
      <ul class="flex w-full lg:justify-start justify-evenly gap-x-8 sm:gap-x-10 gap-y-4 lg:px-4">
        <li v-for="(item, i) in navigation" :key="i" class="interactable">
          <a :href="item.url" target="_blank">
            <Icon :name="item.icon" class="text-xl sm:text-2xl hover:scale-125 lg:hover:scale-150 duration-300" />
          </a>
        </li>
      </ul>
    </TheContainer>
